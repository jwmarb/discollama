...
class Discollama:
    ...
    async def generate(self, content, context):
        ...
        t = datetime.now()
        async for part in await self.ollama.generate(
            model=self.model,
            prompt=content,
            context=context,
            keep_alive=-1,
...
def main():
    ...
    parser.add_argument(
        "--ollama-scheme",
        default=os.getenv("OLLAMA_SCHEME", "http"),
        choices=["http", "https"],
    )
    parser.add_argument(
        "--ollama-host", default=os.getenv("OLLAMA_HOST", "127.0.0.1"), type=str
    )
    parser.add_argument(
        "--ollama-port", default=os.getenv("OLLAMA_PORT", 11434), type=int
    )
    parser.add_argument(
        "--ollama-model", default=os.getenv("OLLAMA_MODEL", "llama2"), type=str
    )

    parser.add_argument(
        "--redis-host", default=os.getenv("REDIS_HOST", "127.0.0.1"), type=str
    )
    ...
    Discollama(
        ollama.AsyncClient(
            host=f"{args.ollama_scheme}://{args.ollama_host}:{args.ollama_port}"
        ),
